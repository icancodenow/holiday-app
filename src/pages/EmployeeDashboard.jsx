...
export default function EmployeeDashboard({ profile }) {
  ...
  async function submitRequest(e) {
    ...
    const { error } = await supabase.from('holiday_requests').insert({
      ...
      end_date: endDate, days_requested: days, reason, leave_type: leaveType
    })
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.colors.background, fontFamily: theme.font.sans }}>
      ...
      <div style={{
        ...
        padding: `0 ${isMobile ? 16 : 32}px`, display: 'flex', alignItems: 'center',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => supabase.auth.signOut()} style={{
            padding: '6px 12px', background: 'white', border: `1px solid ${theme.colors.border}`,
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: isMobile ? '20px 16px' : '32px 24px' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ fontSize: isMobile ? 18 : 20, fontWeight: 600 }}>My holidays</h1>
          <p style={{ color: theme.colors.textSecondary, marginTop: 3, fontSize: 13 }}>{currentYear} overview</p>
        </div>
        ...
        <div style={{
          ...
          {[
            ...
            <div key={i} style={{
              ...
              gridColumn: isMobile && i === 2 ? 'span 2' : 'span 1',
            }}>
              <div style={{ fontSize: 11, color: theme.colors.textMuted, fontWeight: 500, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {s.label}
              </div>
              <div style={{ fontSize: isMobile ? 26 : 32, fontWeight: 700, color: s.color }}>{s.value}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Request holiday</h2>
          <form onSubmit={submitRequest}>
            <div style={{ marginBottom: 14 }}>
              <label style={labelStyle}>Leave type</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {['holiday', 'unpaid'].map(type => (
                  <button key={type} type="button" onClick={() => setLeaveType(type)} style={{
                  ...
                  }}>{type === 'holiday' ? 'Holiday' : 'Unpaid leave'}</button>
                ))}
              </div>
            </div>
            ...
            {message && (
              <div style={{ marginTop: 12, fontSize: 13, color: theme.colors.success }}>{message}</div>
            )}
          </form>
        </div>
        ...
        <div style={{
          ...
          <div style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.colors.border}`, background: '#fafafa' }}>
              ...
              {requests.map(r => (
                <div key={r.id} style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.colors.borderLight}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 500 }}>{r.start_date} – {r.end_date}</div>
                      <div style={{ fontSize: 12, color: theme.colors.textMuted, marginTop: 2 }}>
                        {r.days_requested} days · {r.reason || 'No reason'}
                      </div>
                    </div>
                  ...
                  {r.status === 'pending' && (
                    <button onClick={() => cancelRequest(r.id)} style={{
                      padding: '5px 14px', background: theme.colors.dangerLight,
                      color: theme.colors.danger, border: `1px solid ${theme.colors.dangerBorder}`,
                      borderRadius: theme.radius.full, fontSize: 12, fontWeight: 500, cursor: 'pointer',
...
const tdStyle = { padding: '12px 16px', fontSize: 13, color: '#111827' }